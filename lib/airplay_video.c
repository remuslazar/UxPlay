...
static size_t hls_variant(const char *line, const char *end, const hls_codec_t *codecs,
                          size_t count, bool iframe, uint64_t *pixels) {
    ...
    *pixels = 0;
    ...
    if (!iframe && codecs[selected].fps_milli) {
        unsigned int fps;
...
int select_master_playlist_video(char *playlist, const hls_codec_t *codecs, size_t count) {
    ...
    for (int pass = 0; pass < 2; pass++) {
        ...
        while (*read) {
            ...
            uint64_t pixels = 0;
            size_t codec = stream || iframe ? hls_variant(read, end, codecs, count, iframe, &pixels) : count + 1;
            if (stream) {
                char *uri = next;
